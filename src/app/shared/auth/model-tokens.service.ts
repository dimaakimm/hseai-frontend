import { Injectable } from '@angular/core';
import { HttpClient, HttpParams } from '@angular/common/http';
import {
  BehaviorSubject,
  Observable,
  catchError,
  map,
  of,
  shareReplay,
  switchMap,
  tap,
  throwError,
} from 'rxjs';

import { AuthService } from './auth.service';

export interface ModelTokens {
  access_token: string;
  expires_at?: number; // unix seconds или float (у тебя похоже float)
  expires_in?: number;
  token_type?: string;
  refresh_token?: string;
  refresh_expires_in?: number;
  scope?: string;
  session_state?: string;
  [key: string]: any;
}

export interface GetModelTokensResponse {
  user: any;
  session_created_at: number;
  model_tokens: ModelTokens;
}

@Injectable({ providedIn: 'root' })
export class ModelTokensService {
  private readonly API_BASE = 'https://api.hse-ai.ru';
  private readonly TOKENS_URL = `${this.API_BASE}/get_model_tokens`;

  private readonly tokens$ = new BehaviorSubject<ModelTokens | null>(null);

  // чтобы не дергать /get_model_tokens два раза параллельно
  private inFlight$: Observable<ModelTokens> | null = null;

  constructor(
    private http: HttpClient,
    private auth: AuthService,
  ) {}

  tokensState$(): Observable<ModelTokens | null> {
    return this.tokens$.asObservable();
  }

  clear(): void {
    this.tokens$.next(null);
    this.inFlight$ = null;
  }

  /** Явно обновить токены с бека */
  refreshTokens(): Observable<ModelTokens> {
    if (this.inFlight$) return this.inFlight$;

    const sid = this.auth.getSidOrThrow();
    const params = new HttpParams().set('sid', sid);

    this.inFlight$ = this.http.get<GetModelTokensResponse>(this.TOKENS_URL, { params }).pipe(
      map((res) => res?.model_tokens),
      tap((t) => {
        if (!t?.access_token) {
          throw new Error('model_tokens_missing_access_token');
        }
        this.tokens$.next(t);
      }),
      // shareReplay чтобы несколько подписчиков получили один результат
      shareReplay(1),
      tap({
        finalize: () => {
          // finalize не доступен в tap, поэтому ниже сбросим через catchError/next
        },
      }),
      catchError((err) => {
        this.inFlight$ = null;
        return throwError(() => err);
      }),
      tap({
        next: () => (this.inFlight$ = null),
        error: () => (this.inFlight$ = null),
      }),
    );

    return this.inFlight$;
  }

  /** Проверка токена по expires_at с запасом */
  private hasValidAccessToken(t: ModelTokens | null): t is ModelTokens {
    if (!t?.access_token) return false;
    const exp = t.expires_at;
    if (!exp) return true; // если бек не прислал exp — считаем валидным и ловим 401/403 уже на запросе

    const nowSec = Date.now() / 1000;
    return nowSec < Number(exp) - 10;
  }

  /** Отдать access_token. Если нет/протух — обновить через /get_model_tokens?sid= */
  getAccessToken(): Observable<string> {
    const current = this.tokens$.value;

    if (this.hasValidAccessToken(current)) {
      return of(current.access_token);
    }

    return this.refreshTokens().pipe(map((t) => t.access_token));
  }
}
